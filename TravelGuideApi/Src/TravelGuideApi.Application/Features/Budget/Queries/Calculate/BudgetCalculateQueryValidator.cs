...
public class BudgetCalculateQueryValidator : AbstractValidator<BudgetCalculateQuery>
{
    public BudgetCalculateQueryValidator()
    {
        RuleFor(p => p.HomeCurrency)
            ...
            .Length(3, 3).WithMessage("{PropertyPath} must be a 3-character ISO currency code.");

        RuleFor(p => p.DailyBudget)
            .GreaterThan(0).WithMessage("{PropertyPath} must be greater than 0.");

        RuleFor(p => p.TripDays)
            .GreaterThan(0).WithMessage("{PropertyPath} must be greater than 0.")
            .LessThanOrEqualTo(365).WithMessage("{PropertyPath} must not exceed 365 days.");

        RuleFor(p => p.DestinationCountries)
            .NotEmpty().WithMessage("{PropertyPath} must contain at least one country.");
